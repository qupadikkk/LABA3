...
namespace ConsoleApp1
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Створення нової колекції, що містить таксі й автобуси, які потребують ТО
            List<Transport> transportsForMaintenance = new List<Transport>();

            foreach (Transport transport in savedTransports)
            {
                if (transport is Taxi taxi && taxi.KmCount > 100000)
                {
                    transportsForMaintenance.Add(taxi);
                }
                else if (transport is Bus bus && !bus.IsNew)
                {
                    transportsForMaintenance.Add(bus);
                }
            }

            Console.WriteLine("\nТранспортні засоби, які потребують ТО:");
            foreach (Transport transport in transportsForMaintenance)
            {
                Console.WriteLine(transport);
            }
        }
